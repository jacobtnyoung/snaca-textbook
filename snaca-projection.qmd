...
In the prior chapter on [Bipartite Graphs/Two-Mode Networks](snaca-bipartite.html), saw that not all of the networks we want to examine have a single node set. We discussed how complex relational structures with multiple partitions of node sets can be examined using **bipartite** graphs.
...
To study how police officer's views of body cameras were influenced by those with whom they shared incidents, @young2015 *actually* examined this network:

```{r, fig.cap = "", out.width = "60%"}
knitr::include_graphics( "images/young-ready-projection-figure.jpeg" )
```
...
However, note that we lose some information when we dichotomize the adjacency matrix. Specifically, we are treating a connections that are greater than 1 as being the same as connections exactly equal to 1. This may make sense in some circumstances. For example, suppose you are only concerned with whether two individuals have ever been arrested together. In this case, the dichotomized matrix makes sense to examine.
